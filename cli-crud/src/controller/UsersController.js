...
class UsersController {
  ...
  async deleteUser(id) {
    ...
    /** 
     * Search for users based on a search string and display the results to the user
     * ex: search for "John" will return all users whose name or email contains "John"
     * @async
     * @param {string} searchString - The string to search for users
    */
    ...
    async searchForUsers(searchString) {
        try{
                const users = await usersModel.searchForUsers(searchString);

                if (users.length === 0) {
                    outputHandler.showError("No users found matching your search.");
            } else {
                outputHandler.showUsers(users); // Display the search results in a table format for better readability
                console.log("You're Welcome!");
            }
            
        } catch (error) {
                outputHandler.showError("An error occurred while searching for users.", error);
        }
    }
}
